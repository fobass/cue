# Terms of Service for Cue

**Last Updated: January 21, 2026**

## 1. Agreement to Terms

By downloading, installing, or using the Cue mobile application ("App," "Service," "we," "us," or "our"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the App.

## 2. Description of Service

Cue is a voice-powered reminder application that allows you to:
- Create reminders using voice commands
- Schedule time-based reminders
- Set location-based reminders (Premium feature)
- Create recurring reminders
- Receive notifications for your reminders

### Service Types

**Free Version**
- Up to 5 reminders
- Basic voice recognition
- Time-based reminders
- Daily recurrence
- Standard notifications

**Premium Subscription**
- Unlimited reminders
- Location-based reminders
- Advanced recurrence patterns (weekly, monthly, yearly)
- Background location tracking
- Priority support

## 3. Eligibility

You must be at least 13 years old to use this App. By using the App, you represent and warrant that you meet this age requirement.

## 4. Account and Authentication

### Apple Sign In
- You must sign in with your Apple ID to use the App
- You are responsible for maintaining the security of your Apple account
- You agree to provide accurate and current information
- One account per person

### Account Termination
- You may terminate your account at any time by deleting the App
- We reserve the right to suspend or terminate accounts for violations of these Terms
- Upon termination, your access to Premium features will cease

## 5. Subscription Terms

### Premium Subscription

**Pricing**
- Monthly Subscription: $2.99 USD per month
- Annual Subscription: $19.99 USD per year
- Prices are subject to change with notice

**Billing**
- Payment charged to your Apple ID account at confirmation of purchase
- Subscription automatically renews unless auto-renew is turned off at least 24 hours before the end of the current period
- Account charged for renewal within 24 hours prior to the end of the current period
- Subscriptions may be managed by the user in App Store Account Settings

**Free Trial** (if applicable)
- Free trial period is 7 days (if offered)
- Trial automatically converts to paid subscription unless cancelled
- Cancel anytime during trial period to avoid charges

**Refunds**
- Refund requests are handled by Apple per their refund policy
- Contact Apple Support for refund inquiries
- No refunds for partial subscription periods

**Cancellation**
- Cancel anytime through App Store Account Settings
- Access to Premium features continues until end of billing period
- No partial refunds for unused time

## 6. Acceptable Use

### You Agree To:
- Use the App only for personal, non-commercial purposes
- Provide accurate information when creating reminders
- Comply with all applicable laws and regulations
- Use voice features responsibly and respectfully

### You Agree NOT To:
- Use the App for any illegal purpose
- Attempt to gain unauthorized access to the App or its systems
- Reverse engineer, decompile, or disassemble the App
- Remove or modify any proprietary notices in the App
- Use the App to harass, abuse, or harm others
- Impersonate any person or entity
- Interfere with or disrupt the App's functionality
- Use automated systems (bots) to access the App
- Resell or commercially exploit the App
- Upload malicious code or viruses

## 7. Location Services

### Location-Based Reminders (Premium)

By using location-based reminders, you agree that:
- You will enable location permissions for the App
- The App may access your location in the background
- Location data is used solely for reminder functionality
- You can disable location services at any time
- Location accuracy depends on device capabilities and settings

### Location Accuracy

- We cannot guarantee precise location detection
- Location-based reminders may be delayed or fail due to:
  - Device settings
  - Network connectivity
  - GPS signal strength
  - iOS limitations
  - Environmental factors

## 8. Voice Recognition

### Speech Data

- Voice recordings are processed locally on your device
- Transcripts are sent to our backend AI service for parsing
- We do not store raw audio recordings
- Speech recognition accuracy may vary
- Supported languages may be limited

### Your Responsibilities

- Speak clearly when creating reminders
- Verify reminder details before saving
- We are not responsible for misinterpreted voice commands
- You are responsible for content of voice recordings

## 9. Intellectual Property

### Our Rights

The App and its content, including but not limited to:
- Software code
- Design and layout
- Graphics and icons
- Text and copy
- AI models and algorithms
- Trademarks and logos

are owned by us or our licensors and protected by intellectual property laws.

### Your Rights

- You retain ownership of your reminder content
- You grant us a license to process your data as described in our Privacy Policy
- You may not claim ownership of any App components

## 10. User Content

### Your Reminders

- You are solely responsible for the content of your reminders
- You retain all rights to your reminder content
- We do not claim ownership of your reminders
- We may access your data only as necessary to provide the Service

### Prohibited Content

You may not create reminders containing:
- Illegal content
- Threatening or harassing content
- Content that violates others' rights
- Malicious code or scripts

## 11. Disclaimers and Limitations of Liability

### Service Availability

THE APP IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO:
- Merchantability
- Fitness for a particular purpose
- Non-infringement
- Uninterrupted or error-free operation
- Accuracy or reliability of results

### Not a Substitute

CUE IS A REMINDER APP ONLY. IT IS NOT:
- A medical alert system
- An emergency notification service
- A safety or security system
- A substitute for professional advice

DO NOT RELY ON CUE FOR:
- Medical reminders critical to health
- Time-sensitive legal or financial deadlines
- Emergency situations
- Life-safety applications

### Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW:

WE ARE NOT LIABLE FOR:
- Lost profits or data
- Missed reminders or notifications
- Inaccurate location detection
- Device or network issues
- Third-party service failures
- Indirect, incidental, or consequential damages

MAXIMUM LIABILITY:
- Our total liability shall not exceed the amount you paid for Premium subscription in the past 12 months, or $50 USD, whichever is less

### Location-Based Reminder Limitations

WE ARE NOT LIABLE FOR:
- Inaccurate location detection
- Failed geofence triggers
- Delayed notifications
- Battery drain from location services
- iOS location service limitations

## 12. Indemnification

You agree to indemnify and hold us harmless from any claims, damages, losses, liabilities, and expenses (including legal fees) arising from:
- Your use of the App
- Your violation of these Terms
- Your violation of any rights of another party
- Your reminder content

## 13. Third-Party Services

The App integrates with third-party services:
- **Apple Services** (Sign in with Apple, StoreKit, CloudKit)
- **Backend AI Service** (reminder parsing)
- **DigitalOcean** (hosting infrastructure)

These services have their own terms and privacy policies. We are not responsible for third-party service interruptions or issues.

## 14. Changes to the App

We reserve the right to:
- Modify or discontinue features
- Update the App at any time
- Change Premium subscription pricing with notice
- Suspend the Service for maintenance

We will notify you of material changes through:
- In-app notifications
- App Store update notes
- Email (if provided)

## 15. Changes to Terms

We may update these Terms at any time. Changes are effective when posted. Your continued use of the App after changes constitutes acceptance of the new Terms.

We will notify you of material changes via:
- In-app notification
- Updated date at the top of these Terms
- Email notification (for Premium subscribers)

## 16. Termination

### By You
- You may stop using the App at any time
- Delete the App to remove all local data
- Cancel Premium subscription in App Store settings

### By Us

We may suspend or terminate your access if:
- You violate these Terms
- You engage in fraudulent activity
- You abuse the Service
- Required by law

Upon termination:
- Your access to Premium features ends immediately
- Local data remains on your device until you delete the App
- No refunds for unused subscription time

## 17. Governing Law and Dispute Resolution

### 17.1 Governing Law
These Terms are governed by the laws of the Kyrgyz Republic.

### 17.2 Dispute Resolution
We aim to resolve any issues amicably. Please contact us at isabekove@gmail.com with any concerns before pursuing formal action. We will make our best effort to address your concerns within 30 days.

### 17.3 Jurisdiction
Any disputes that cannot be resolved amicably shall be subject to the exclusive jurisdiction of the courts of Chuy Region, Kyrgyz Republic.

## 18. App Store Terms

Your use of the App is also subject to:
- Apple's App Store Terms of Service
- Apple's Licensed Application End User License Agreement (EULA)
- Your agreement with your mobile carrier

In case of conflict, these Terms prevail regarding the App itself.

## 19. Export Compliance

You agree to comply with all export and import laws and regulations. You may not use or export the App in violation of U.S. export laws and regulations.

## 20. Severability

If any provision of these Terms is found to be invalid or unenforceable, that provision will be limited or eliminated to the minimum extent necessary, and the remaining provisions will remain in full effect.

## 21. No Waiver

Our failure to enforce any right or provision of these Terms will not be considered a waiver of those rights.

## 22. Entire Agreement

These Terms, along with our Privacy Policy, constitute the entire agreement between you and us regarding the App.

## 23. Contact Information

For questions about these Terms:

**Email**: isabekove@gmail.com

## 24. Apple-Specific Terms

### Acknowledgment

You acknowledge that:
- These Terms are between you and us, not Apple
- We, not Apple, are responsible for the App and its content
- Apple has no obligation to provide support for the App
- Apple is not responsible for addressing claims about the App

### Third-Party Beneficiary

Apple and its subsidiaries are third-party beneficiaries of these Terms and may enforce these Terms against you.

### Representation and Warranty

You represent that:
- You are not located in a country subject to U.S. Government embargo
- You are not on any U.S. Government list of prohibited parties
- You will comply with all applicable third-party terms when using the App

## 25. Privacy

Your use of the App is also governed by our Privacy Policy, available at https://fobass.github.io/cue/PRIVACY_POLICY. By using the App, you consent to our Privacy Policy.

## 26. Feedback

If you provide feedback, suggestions, or ideas about the App:
- We may use them without obligation to you
- You grant us a perpetual, worldwide license to use feedback
- We are not required to keep feedback confidential

---

## Summary (Not Legally Binding)

**In Plain English:**
- Use the App responsibly and legally
- Free tier has 5 reminders, Premium is unlimited
- Premium costs $2.99/month or $19.99/year
- We may change prices or features with notice
- Don't rely on Cue for critical or emergency reminders
- We're not liable for missed reminders
- You can cancel Premium anytime
- Contact us if you have issues

**Remember**: This summary is for convenience only. The full Terms above are the binding agreement.

---

**By using Cue, you agree to these Terms of Service and our Privacy Policy.**

**Thank you for using Cue! 🎙️**
